...
def clip_l2_diff(clip):
    """
    @param clip: A numpy array of shape [c.TRAIN_HEIGHT, c.TRAIN_WIDTH, (3 * (c.HIST_LEN + c.GT_LEN))].
...
def process_clip():
    ...
    @return: An array of shape [c.TRAIN_HEIGHT, c.TRAIN_WIDTH, (3 * (c.HIST_LEN + c.GT_LEN))].
             A frame sequence with values normalized in range [-1, 1].
    """
    while True:
        try:
            clip = get_full_clips(c.TRAIN_DIR, c.GT_LEN)[0]
            break
        except:
            continue
    ...
    cropped_clip = np.empty([c.TRAIN_HEIGHT, c.TRAIN_WIDTH, 3 * (c.HIST_LEN + c.GT_LEN)])
...
def get_train_batch():
    ...
    @return: An array of shape
            [c.BATCH_SIZE, c.TRAIN_HEIGHT, c.TRAIN_WIDTH, (3 * (c.HIST_LEN + c.GT_LEN))].
    """
    clips = np.empty([c.BATCH_SIZE, c.TRAIN_HEIGHT, c.TRAIN_WIDTH, (3 * (c.HIST_LEN + c.GT_LEN))],
                     dtype=np.float32)
...
def get_test_batch(test_batch_size, num_rec_out=1, reverse=False):
    """
    ...
    @param num_rec_out: The number of outputs to predict. Outputs > 1 are computed recursively,
                        using the previously-generated frames as input. Default = 1.
    @param reverse: Reverse the image order in test batch
    ...
    """
    if reverse:
        return reverse_sampled_data(get_full_clips(c.TEST_DIR, test_batch_size, num_rec_out=num_rec_out))
    else:
        get_full_clips(c.TEST_DIR, test_batch_size, num_rec_out=num_rec_out)


def reverse_sampled_data(clips):
    rev_clips = []

    height = len(clips[0])
    width = len(clips[0][0])
    length = len(clips[0][0][0])

    for i in range(len(clips)):
        rev_clip = np.empty([height, width, length])

        clip = clips[i]

        for j in range(0, len(clip)):
            for k in range(0, len(clip[j])):
                rev_frame = reverse(np.flip(clip[j, k], 0), length, 3)
                rev_clip[j][k] = rev_frame

        rev_clips.append(rev_clip)

    return np.array(rev_clips)
...
def ssim_error(gen_frames, gt_frames):
    with tf.name_scope('ssim'):
        return tf.reduce_mean(tf.image.ssim(gen_frames, gt_frames, max_val=1))
