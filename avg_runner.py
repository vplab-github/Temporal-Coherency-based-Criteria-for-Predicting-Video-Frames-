...
class AVGRunner:
    def __init__(self, num_steps, model_load_path, num_test_rec):
        ...
        self.g_model = GeneratorModel(self.sess,
                                      ...
                                      c.SCALE_IS_UNPOOLING_G,
                                      c.SCALE_IS_BATCH_NORM_G,
                                      c.SCALE_GT_INVERSE_SCALE_FACTORS)
    ...
    def test(self):
        ...
        """
        batch = get_test_batch(c.BATCH_SIZE, num_rec_out=self.num_test_rec, reverse=True)
...
def usage():
    ...
    print 'Note: Change c.HIST_LEN to desired value before running this code'
...
def main():
    ...
    try:
        opts, _ = getopt.getopt(sys.argv[1:], 'l:t:r:a:n:s:OTH',
                                ['load_path=', 'test_dir=', 'recursions=', 'adversarial=', 'name=',
                                 'steps=', 'overwrite', 'test_only', 'help',
                                 'stats_freq=', 'summary_freq=', 'img_save_freq=', 'test_freq=',
                                 'model_save_freq='])
    ...
    c.reset_scale_fms_g_2()

    if c.GT_LEN <= 1:
        print 'Number of recursions must be greater than 1!'
        return
